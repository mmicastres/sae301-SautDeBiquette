...
class UtilisateurManager
    {
		...
		* @return int le nb d'utilisateurs
		*/
		public function count():int {
			$stmt = $this->_db->prepare('SELECT COUNT(*) FROM utilisateur');
			$stmt->execute();
			return $stmt->fetchColumn();
		}

		
		public function getIdUtilisateurMail($mail){	
	...
	*/
	public function participeUtilisateur($idUtilisateur) : bool {
		$utilisateur = array();
		$req = "SELECT * FROM participe WHERE id_utilisateur = ? ";
		$stmt = $this->_db->prepare($req);
		$res = $stmt->execute(array($idUtilisateur));
		while ($donnees = $stmt->fetch())
		{
			$utilisateurs[] = new Utilisateur($donnees);
		}
		// je vérifie que ce ne soit pas vide ou null 
		return empty($utilisateurs);
